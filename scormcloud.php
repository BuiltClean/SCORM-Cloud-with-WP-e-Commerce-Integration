<?php 
	...
	Author: BuiltClean/eLearning Enhanced
	Version: 1.0.1
	Author URI: http://elearningenhanced.com
	*/
	...
	function scormcloud_admin_actions() {
        ...
        wp_enqueue_script('scormcloud-admin', plugins_url('/wp-e-commerce-scormcloud/scripts/scormcloud.admin.js'), array() );
        wp_enqueue_script('scormcloud-date_format', plugins_url('/wp-e-commerce-scormcloud/scripts/date.format.js'), array() );
		...
		add_menu_page('SCORM Cloud Overview', 'SCORM Cloud', 8, __FILE__, 'scormcloud_toplevel_page');
		//add_submenu_page(__FILE__, 'SCORM Engine - Import New Package', 'Import New Package', 8, 'importpackage', 'scormcloud_import_package');
		add_submenu_page(__FILE__, 'SCORM Cloud Courses', 'Courses', 8, 'scormcloudcourses', 'scormcloud_manage_courses');
		add_submenu_page(__FILE__, 'SCORM Cloud Training', 'Training', 8, 'scormcloudtraining', 'scormcloud_manage_training');
		add_submenu_page(__FILE__, 'SCORM Cloud/WP e-Commerce', 'WP e-Commerce', 8, 'scormcloudwpecommerce', 'scormcloud_manage_wp_ecommerce');
        ...
        if (!scormcloud_isScormCloudNetworkManaged() || (scormcloud_isScormCloudNetworkManaged() && is_super_admin())){
            add_submenu_page(__FILE__, 'SCORM Cloud Settings', 'Settings', 8, 'scormcloudsettings', 'scormcloud_settings');
        }
        require_once('scormcloud_contextualHelp.php');
        add_action( 'contextual_help', 'scormcloud_contextualHelp',10,2 );
	}
    ...
    function scormcloud_embed_plugin( $plugins ) {
        $plugins['scormCloudEmbed'] = plugins_url('/wp-e-commerce-scormcloud/tinymce3/editor_plugin.js');
        return $plugins;
    }
    ...
    wp_enqueue_script( 'scormclouddialog', plugins_url('/wp-e-commerce-scormcloud/scripts/scormcloud.dialog.js'), array() );
...
// <![CDATA[
	if (typeof ScormCloud !== 'undefined' && typeof ScormCloud.Dialog !== 'undefined') {
        var dialogUrl = "<?php echo plugins_url('/wp-e-commerce-scormcloud/embedTraining.php'); ?>";
...
<?php	
	...
	function scormcloud_manage_wp_ecommerce(){
		include('scormcloud_manage_wp_ecommerce.php');
	}
    ...
    */
    ...
    wp_enqueue_script('scormcloud-post', plugins_url('/wp-e-commerce-scormcloud/scripts/scormcloud.post.js'), array() );
    wp_register_style('scormcloud-post-style', plugins_url('/wp-e-commerce-scormcloud/css/scormcloud.post.css'));
    wp_enqueue_style('scormcloud-post-style');
    add_filter('the_content','scormcloud_addTrainingHTML',15);
