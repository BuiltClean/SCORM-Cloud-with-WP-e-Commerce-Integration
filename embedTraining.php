...
<h1><?php _e("Add training to your post"); ?></h1>


<span class="labelheader"><?php _e("First select a course"); ?>: </span>
...
<div class='selectOptionsDiv'>
    <span class="labelheader"><?php _e("Next select some options"); ?>: </span>
    <table>
        <tr>
            <td class="label"><?php _e("Training Header Text"); ?>:</td>
            <td><input type="text" name="trainingHeaderTxt" value="<?php _e("Launch Training Now"); ?>"/></td>
        </tr>
        <tr>
            <td class="label"><?php _e("Training Description"); ?>:</td>
            <td><textarea type="text" name="trainingDesc" ><?php _e("Click 'Start Training' to take your training."); ?></textarea></td>
        </tr>
        <tr>
            <td colspan='2'><input type="checkbox" checked name="trainingRequireLogin"/><?php _e("Require that learners be authenticated users."); ?></td>
        </tr>
        <tr>
            <td colspan='2'><input type="checkbox" checked name="showCourseInfo"/><?php _e("Show the course title and description."); ?></td>
        </tr>
...
<input type="button" class="generateTrainingTag button" name="generateTrainingTag" value="<?php _e("Embed This Training"); ?>" />

<a href="javascript:void(0);" class="toggleButton previewPostEmbed" toggleobject='#embedTrainingDialog .previewDiv' onText='<?php _e("hide preview"); ?>' offText='<?php _e("show preview"); ?>'><?php _e("show preview"); ?></a>
<div class="previewDiv">
    <div class="scormCloudInvitation">
        <h3><?php _e("Launch Training Now"); ?></h3>
        <p class="description"><?php _e("Click 'Start Training' to take your training."); ?></p>
        <div class="courseInfo">
            <div class="title"></div>
            <div class="desc"><?php _e("This will be the metadata description for your course (if it exists).  Also, the displayed duration will render if it exists in the metadata."); ?></div>
            <div class="duration"><?php _e("Duration: 10 minutes"); ?></div>
        </div>
        <p class="inputs">
            <?php _e("My name is"); ?> <input disabled name="scormcloudfname" placeholder="First Name" type="text" >
            <input name="scormcloudlname" disabled placeholder="Last Name" type="text" >
                <?php _e("and my email is"); ?> <input name="scormcloudemail" disabled placeholder="Email" type="text"> .</p>
    ...
    $j(document).ready(function(){
        ...
        $j('#embedTrainingDialog .courseSelector').change(function(){
            ...
            if (selObj.val() != ''){
                ...
                $j('#embedTrainingDialog .previewDiv div.courseInfo div.title').text('<?php _e("Title"); ?>: ' + selObj.text());
...
<?php
     ...
     echo "<div>
            <h2>Please configure your SCORM Cloud settings to add training to your posts.</h2>
        </div>";
