...
function scormcloud_makeBlogEntry($content){
    ...
    foreach($cloudTags as $tagString){
        ...
        if (!$isValidAccount || $invite->active != 1){
            $inviteHtml .= "<h3>".__("This training is not currently active.")."</h3>";
        } else {
            ...
            if($current_user->user_login == '')
            {
                if ($regsRemaining > 0){
                    if ($invite->require_login == 0){
                        ...
                        $inviteHtml .= "<input name='launch' type='button' key='$inviteId' onclick='ScormCloud.Post.makeAnonRegLaunch(\"$inviteId\");' url='" .get_option( 'siteurl' ) ."/wp-content/plugins/wp-e-commerce-scormcloud/ajax.php' value='Start Training'/>";
                    } else {
                        $inviteHtml .= "<h3>".__("Please log in to take this training.")."</h3>";
                    }
                } else {
                    $inviteHtml .= "<h3>".__("This training is not currently active.")."</h3>";
                }
                ...
                if (count($regs) > 0){
                    ...
                    "<td class='head'>".__("Completion")."</td>" .
                    "<td class='head'>".__("Success")."</td>" .
                    "<td class='head'>".__("Score")."</td>" .
                    "<td class='head'>".__("Total Time")."</td>" .
                    "</tr><tr>" .
                    "<td class='$completion'>".__($completion)."</td>" .
                    "<td class='$success'>".__($success)."</td>" .
                    "<td class='".__($score == "unknown" ? "uknown" : "")."'>".__($score == "unknown" ? "-" : $score."%")."</td>".
                    "<td class='time'>$seconds secs</td>" .
                    "</tr></table>";
                    
                    
		$inviteHtml .= "<input name='launch' type='button' key='$inviteId' onclick='ScormCloud.Post.getLaunchURL(\"$inviteId\",\"$regId\");' url='" .get_option( 'siteurl' ) ."/wp-content/plugins/wp-e-commerce-scormcloud/ajax.php' value='Relaunch Training'/>";
                    
                }else{
                    if ($regsRemaining > 0){
			if(!get_site_option('scormcloud_wpecommerce')) {
	                        $inviteHtml .= "<input name='launch' type='button' key='$inviteId' onclick='ScormCloud.Post.makeUserRegLaunch(\"$inviteId\");' url='" .get_option( 'siteurl' ) ."/wp-content/plugins/wp-e-commerce-scormcloud/ajax.php' value='Start Training'/>";
			}
                    } else {
                        $inviteHtml .= "<h3>".__("This training is not currently active.")."</h3>";
                    }
