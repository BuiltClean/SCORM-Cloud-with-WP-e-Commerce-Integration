...
require_once('scormcloud.wp.php');
    ...
    if($_POST['scormcloud_hidden'] == 'Y') {
		//Form data sent
		$dbhost = $_POST['scormcloud_dbhost'];
        $dbname = $_POST['scormcloud_dbname'];
        $dbuser = $_POST['scormcloud_dbuser'];
        $dbpwd = $_POST['scormcloud_dbpwd'];
        ...
        if (!$ScormService->isValidUrl()){
            echo "<div class='updated'><p class='failed'><strong>". __("Invalid Service Url.  Check your URL and try again or clear out the bad URL and the default URL will be used.")."</strong></p></div>";
        } elseif (!$ScormService->isValidAccount()){
            echo "<div class='updated'><p class='failed'><strong>". __("Invalid Credentials.  Check your App Id and Secret Key and try again.")."</strong></p></div>";
        } else {
        ...
        if (!$ScormService->isValidUrl()){
            echo "<div class='updated'><p class='failed'><strong>". __("Invalid Service Url.  Check your URL click 'Update Options' or clear out the bad URL and the default URL will be used.")."</strong></p></div>";
        } elseif (!$ScormService->isValidAccount()){
            echo "<div class='updated'><p><strong>". __("Please fill in your SCORM Cloud credentials and click 'Update Options'.")."</strong></p></div>";
        } 
...
<div class="wrap">
	...
	<?php    echo "<h2>" . __( 'Rustici Software SCORM Cloud Settings' ) . "</h2>"; ?>
	<p><em><?php _e("The following settings require the credentials provided when you signed up for your SCORM Cloud account.  They can also be found by going to your Apps page in the SCORM Cloud site." ); ?>
    <br/><?php _e("If you need an account on SCORM Cloud, "); ?><a href="https://cloud.scorm.com" target="_blank" title="<?php _e("Open the SCORM Cloud Site in a new window."); ?>"><?php _e("click here to go there now"); ?></a>.</em></p>
	...
	<form name="scormcloud_form" method="post" action="<?php echo str_replace( '%7E', '~', $_SERVER['REQUEST_URI']); ?>">
		...
		<p><?php _e("Cloud Engine URL: " ); ?><input type="text" name="scormcloud_engine_url" value="<?php echo $engine_url; ?>" size="50"></p>
		
		<p><?php _e("App ID: " ); ?><input type="text" name="scormcloud_appid" value="<?php echo $appid; ?>" size="20">
		<br/><em><?php _e(" ex: MYAYX1KB3G" ); ?></em></p>
		<p><?php _e("Secret Key: " ); ?><input type="text" name="scormcloud_secretkey" value="<?php echo $secretkey; ?>" size="50">
		<br/><em><?php _e(" ex: fJ4Q2HwEZbhRLeftHLv0SjzIVpJLXRiyfyOiWBo0" ); ?></em></p>
        
        <br/>
        <p><?php _e("SCORM Player Stylesheet Url: " ); ?><input type="text" name="scormcloud_player_cssurl" value="<?php echo $player_cssurl; ?>" size="50">
		<br/><em><?php _e(" ex: http://cloud.scorm.com/sc/css/cloudPlayer/cloudstyles.css" ); ?></em></p>
    <?php 
    if (scormcloud_isScormCloudNetworkManaged()){ ?>
        <p><input type="checkbox" name="scormcloud_sharecourses" <?php echo ($sharecourses ? "checked" : ""); ?> /><?php _e(" Share courses among all sites." ); ?></p>
    <?php } ?>
		<p class="submit">
...
<!--<iframe class='signupFrame' src='https://cloud.scorm.com/sc/embedded/EmbeddedSignUpForm?cssUrl=<?php //echo plugins_url('/wp-e-commerce-scormcloud/css/scormcloud.admin.css'); ?>'/>-->
